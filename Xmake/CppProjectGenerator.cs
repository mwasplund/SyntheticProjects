...
namespace SyntheticProjects.Xmake
{
	public class CppProjectGenerator
	{
		private static Path XMakeFileName => new Path("xmake.lua");
		private static string XMakeFileFormat => @"add_rules(""mode.debug"", ""mode.release"")
...
";
		...
		private async Task GenerateProjectAsync(Path directory, string name, IList<Path> references)
		{
			...
			await WriteXMakeFileAsync(directory, name, references, files);
		}
		...
		private static async Task WriteXMakeFileAsync(
			Path directory,
			string name,
			...
			IList<Path> source)
		{
			...
			var fileContent = string.Format(
				...
				name,
				referencesList);
			var filePath = directory + XMakeFileName;
